...
huggingface_token = "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"
...
DEFAULT_TEST_CONFIG = 'imdb_pwws_distilbert.csv'
...
parser.add_argument('--test_config', type=str, default=DEFAULT_TEST_CONFIG, help='Test configuration file')
...
def load_hugging_face_model(model_arch, dataset):
    try:
        if model_arch == 'distilbert':
            ...
            model_name = f"textattack/{model_arch}-base-uncased-{dataset}"
        elif model_arch == 'bert':
            from transformers import AutoTokenizer, AutoModelForSequenceClassification
            model_name = f"textattack/bert-base-uncased-{dataset}"
        else:
            raise ValueError(f"Unsupported model architecture: {model_arch}")

        tokenizer = AutoTokenizer.from_pretrained("textattack/distilbert-base-uncased")
        model = AutoModelForSequenceClassification.from_pretrained("textattack/distilbert-base-uncased")
