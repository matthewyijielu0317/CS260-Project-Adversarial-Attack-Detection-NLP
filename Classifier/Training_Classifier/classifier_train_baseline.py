...
test_config = 'ag-news_pwws_distilbert_logits_wo_filtering.csv'

# import os
# file_path = f'../../Generating Adversarial Samples/Data/{test_config}'
# print("Resolved Path:", os.path.abspath(file_path))

test_downstream_coinfig = [
    "ag-news_alzantot_distilbert_logits_wo_filtering.csv", 
    "ag-news_textfooler_distilbert_logits_wo_filtering.csv", 
    "imdb_bae_distilbert_logits_wo_filtering.csv", 
    "rotten-tomatoes_alzantot_distilbert_logits_wo_filtering.csv", 
]
...
for i, df in enumerate(df_test_downstream):
    print(test_downstream_coinfig[i], df.shape)  
    print(df.head(5))


# Divide train and test set
df_train = df.head(int(len(df) * 0.9))
df_test = df.tail(int(len(df) * 0.1)) 
...
# print the number of non-zero elements in each item of x_test_downsteam 
for i, x in enumerate(x_test_downstream):
    print('Downstream Dataset:', test_downstream_coinfig[i], np.count_nonzero(x)) 
...
for i, (x_test_ds, y_test_ds) in enumerate(zip(x_test_downstream, y_test_downstream)):
    ...
    print(np.sum(xgb_predictions_ds==y_test_ds)/len(y_test_ds)) 
